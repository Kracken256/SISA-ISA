...
static void sisa16_syscall_halt(sisa16_cpu_t* cpu) {
  cpu->halted = 1;
  printf("Machine halted\n");
}

#define SISA_SYS_WRITE 0x01
static void sisa16_syscall_write(sisa16_cpu_t* cpu) {
...
int main(int argc, char** argv) {
  ...
  SISA_SYSCALL(cpu, SISA_SYS_WRITE, sisa16_syscall_write);
  SISA_SYSCALL(cpu, SISA_SYS_HALT, sisa16_syscall_halt);
